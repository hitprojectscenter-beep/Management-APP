      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {isHe ? "חיזוי AI - תאריך סיום צפוי" : "AI Forecast - Predicted Completion"}
          </CardTitle>
        </CardHeader>
        <CardContent className="text-center py-8 text-sm text-muted-foreground">
          {isHe ? "אין מספיק נתונים לחיזוי" : "Not enough data to forecast"}
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="ms-auto">
            ...
            {forecast.confidence === "high"
              ? isHe ? "ביטחון גבוה" : "High confidence"
              : forecast.confidence === "medium"
                ? isHe ? "ביטחון בינוני" : "Medium confidence"
        ...
        <CardDescription className="flex items-start gap-1.5">
          ...
          <span>
            {isHe
              ? "ה-AI לומד מהיסטוריית הביצועים: קצב סגירת משימות, ממוצע פיגור, וחריגת שעות. על בסיס זה הוא חוזה את תאריך הסיום הריאליסטי."
              : "AI learns from past performance: completion rate, average slip, and hour overruns. Based on this it forecasts a realistic end date."}
          </span>
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className={cn("p-4 rounded-lg border-2", accentBg)}>
            <div className="flex items-center gap-2 text-xs uppercase mb-2">
              ...
              <span className={accent}>{isHe ? "צפוי AI" : "AI Forecast"}</span>
            </div>
          ...
          <div className={cn("p-4 rounded-lg border-2", accentBg)}>
            <div className="flex items-center gap-2 text-xs uppercase mb-2">
              ...
              <span className={accent}>{isHe ? "סטיית AI" : "AI Variance"}</span>
            </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="p-3 rounded-md border bg-background flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] text-muted-foreground uppercase">{isHe ? "מהירות בפועל" : "Actual velocity"}</div>
              <div className="text-sm font-bold">
                {forecast.velocityActual.toFixed(1)} {isHe ? "משימות/שבוע" : "tasks/week"}
              </div>
            </div>
          </div>
          <div className="p-3 rounded-md border bg-background flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] text-muted-foreground uppercase">{isHe ? "מהירות נדרשת" : "Required velocity"}</div>
              <div className="text-sm font-bold">
                {forecast.velocityRequired.toFixed(1)} {isHe ? "משימות/שבוע" : "tasks/week"}
              </div>
            </div>
          </div>
        </div>
