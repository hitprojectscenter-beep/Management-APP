...
> מסמך זה מסכם את העקרונות, הכלים, ההחלטות והשיטות שבהן השתמשתי (Claude) במהלך פיתוח **PMO++**. הוא משמש כ"מדריך הפעלה" לכל מי שיבוא להמשיך את הפיתוח (כולל גרסאות עתידיות שלי).
...
2. **שילוב הלוגו ב-sidebar header** - באותו מקום שהיה הקוביה הכחולה הקודמת. עם רקע לבן וכותרת "המרכז למיפוי ישראל" + תת-כותרת PMO++
...
## 🆕 סבב פיתוח 26 — Gemini AI, יומן פעילות, זיהוי שפה

### חלק א': יומן פעילות משתמשים (Activity Log)
- **`lib/db/activity-log.ts`** — טיפוסי `ActivityEntry` ו-`UserStats`, 40+ פריטי פעילות דמו
- **`computeUserStats(userId)`** — כניסות, פעולה אחרונה, פעולות השבוע, משימות נוצרו/עודכנו/הושלמו
- **`components/admin/activity-log.tsx`** — שתי תצוגות:
  - **סיכום לפי משתמש**: כרטיס לכל משתמש עם סטטיסטיקות + פריסה של פעילות אחרונה
  - **ציר זמן**: 30 פעולות אחרונות כרונולוגית עם אייקונים צבעוניים לפי סוג
- **טאב 6 בניהול מערכת**: "יומן פעילות" / "Activity Log"

### חלק ב': תיקון מעבר בין שפות
- **בעיה**: המערכת נעולה על רוסית, לא ניתן לעבור לעברית
- **סיבה**: `localeDetection: true` ב-middleware — קריאה של `Accept-Language` header בכל בקשה דרסה את הבחירה
- **פתרון**: `localeDetection: false` — השפה נקבעת לפי: URL prefix → Cookie → ברירת מחדל (עברית)

### חלק ג': עוזר אישי עם זיהוי שפה חכם
- **זיהוי שפה מהטקסט**: אם המשתמש כותב בעברית (תווי `\u0590-\u05FF`) — העוזר עונה בעברית, בלי קשר ל-locale
- **זיהוי באנגלית**: רק אותיות לטיניות → תשובה באנגלית
- **עדיפות**: 1) תווי טקסט 2) locale 3) ברירת מחדל עברית

### חלק ד': אינטגרציית Gemini API
- **`lib/ai/gemini.ts`** — client חדש ל-Google Gemini 2.0 Flash
- **`askGemini(question, context, isHebrew)`** — wrapper פשוט עם Knowledge Base כקונטקסט
- **Fallback חכם ב-API**: אם ה-heuristic החזיר "unknown" או השאלה נראית אינפורמטיבית (מכילה "?", "איך", "מה", וכו'), העוזר משתמש ב-Gemini לתשובה חכמה
- **System Instruction בעברית/אנגלית** — Gemini תמיד עונה בשפת המשתמש
- **מפתח נקרא מ-`GEMINI_API_KEY`** או `GOOGLE_API_KEY` (fallback)

### חלק ה': חיפוש Knowledge Base משופר
- **5 שיטות חיפוש במקביל**:
  1. התאמת מחרוזת (boost כפול)
  2. התאמת מילה-למילה (cross-matching)
  3. דמיון לטקסט השאלה (עברית + אנגלית)
  4. סריקת טקסט התשובה
  5. התאמת קטגוריה (mapping של מונחים לקטגוריות)
- **Regex מורחב**: תופס מונחי דומיין ישירות (גאנט, wbs, kpi, דשבורד, אוטומ, יומן, סיכון, פרויקט, משימ)
- **UNKNOWN handler**: ניסיון חיפוש ב-KB לפני ויתור

### חלק ו': הערה חשובה לפריסה
- קובץ `.env.local` **לא** מועלה ל-git (ב-.gitignore)
- כדי שה-Gemini יעבוד ב-Vercel Production, יש להוסיף `GEMINI_API_KEY` ב-Vercel Environment Variables

---

## 🆕 סבב פיתוח 27 — תיקון Gemini + זיהוי שפה מטקסט

### בעיה מרכזית: מודל Gemini לא קיים
- **gemini-2.0-flash-exp** הוסר ע"י Google → כל קריאה החזירה 404
- Gemini נפל בשקט ל-heuristic → תשובות באנגלית
- **פתרון**: שרשרת 3 מודלים: `gemini-2.5-flash` → `gemini-2.0-flash-lite` → `gemini-2.0-flash`
- נבדק ואושר: המפתח פעיל, gemini-2.5-flash עובד ועונה בעברית

### זיהוי שפה מהטקסט (לא מ-locale בלבד)
- **`detectLang(text, locale)`** — מזהה שפה מתווי הטקסט:
  - עברית: `\u0590-\u05FF` → "he"
  - Cyrillic: `\u0400-\u04FF` → "ru"
  - Latin + locale → en/fr/es
  - Default → "he"
- **הודעת פתיחה**: עברית לכל locale שאינו "en" (במקום רק locale==="he")
- **TTS**: שפת דיבור מזוהה מהתשובה, לא מ-locale

---

## 🆕 סבב פיתוח 28 — תמיכה מלאה ב-5 שפות (עוזר + בוט + Gemini)

### עוזר אישי — 5 שפות מלאות
- **WELCOME_MESSAGES**: הודעת פתיחה ב-he/en/ru/fr/es
- **FALLBACK**: noData, helpMore, error, confirm — 5 שפות
- **TTS_LANG**: קודי שפה לדיבור (he-IL, en-US, ru-RU, fr-FR, es-ES)

### Gemini AI — הוראות שפה ייעודיות
- **GEMINI_LANG_CONFIG** — 5 שפות, כל אחת עם:
  - `directive`: אכיפת שפה מפורשת (למשל `"Отвечай только на русском"`)
  - `instruction`: System prompt בשפה
  - `prefix`: תחילית הודעת משתמש (למשל `"[Instrucción: Responde solo en español]"`)
- `askGemini()` מקבל `lang: string` (לא `isHebrew: boolean`)

### API Route — זיהוי 5 שפות
- `responseLang`: Hebrew chars → he, Cyrillic → ru, locale fr/es/en → locale, default → he
- Regex `isQuestionLike` הורחב: רוסית (как, что, почему), צרפתית (comment, qu'est), ספרדית (cómo, qué)

### בוט עזרה — 5 שפות
- **BOT_LABELS**: כותרת, כותרת משנה, פתיחה, placeholder, הצעות, שגיאות — 5 שפות
- **QUICK_QUESTIONS**: 4 שאלות × 5 שפות = 20 שאלות מתורגמות
- כל ה-`isHe` ternaries הוחלפו ב-`labels[lang]`

### heuristicParse — זיהוי 5 שפות
- `hasHebrewChars`, `hasCyrillicChars` — זיהוי Unicode
- `isHe`, `isRu`, `isFr`, `isEs` — flags לשליטה בשפת תשובה
- ה-heuristic עונה בעברית/אנגלית, Gemini עונה ב-5 שפות

---

## 🆕 סבב פיתוח 29 — משימות שנוצרות במשתמש עוברות ל-PostgreSQL (cross-device, per-user)

### הבעיה
משימות שנוצרו ב-UI (טופס "משימה חדשה" + מרכז החילוץ) נשמרו **רק ב-localStorage** של הדפדפן.
התוצאה: משימה נראתה רק במכשיר שבו נוצרה, וקישור ישיר אליה החזיר 404 בכל מקום אחר.

### הפתרון — מצב היברידי מכוון
- **נשאר mock-data**: הנתונים הזרועים לדמו (פרויקטים, WBS, sf-task-*) — מרונדרים בשרת.
- **עבר ל-PostgreSQL**: משימות שהמשתמש יוצר. טבלה חדשה `app_tasks` (Neon).

### סכמה — `lib/db/schema.ts`
- `app_tasks` משקפת את צורת `MockTask`: `id` טקסט, תאריכים כ-**טקסט** ("yyyy-mm-dd" — round-trip
  מדויק, בלי הסחת timezone של `Date()`), jsonb ל-tags/dependencies/resources/attachments/sourceFile.
- `creator_id`/`assignee_id` טקסט **בלי FK** — המשתמשים הזרועים הם u1..u6, אבל מוזמנים עשויים להיות
  client-only, אז כתיבה לא נכשלת על id לא-זרוע. ההרשאות נאכפות ב-repo, לא ב-FK.
- הוחל ב-`drizzle-kit push` (אדיטיבי, ללא prompts).

### שכבת גישה — server-only, מוגן-סשן, runtime nodejs
- **`lib/db/app-tasks-repo.ts`** — list/get/upsert/bulk/delete + מודל ההרשאות (נאכף בשרת):
  - admin → הכל
  - manager → שלו + מה ששויך אליו + **כל תת-עץ הכפיפות** (managerId טרנזיטיבי)
  - member/viewer/guest → רק שלו + מה ששויך אליו
- **`lib/auth/require-user.ts`** — פתרון סשן למשתמש פעיל (כל תפקיד).
- **`/api/tasks`** (GET/POST), **`/api/tasks/[id]`** (GET/PATCH/DELETE). `creatorId` תמיד מהסשן, לעולם לא מה-body.

### צד-לקוח — DB-backed עם localStorage כ-cache אופטימי/אופליין — `lib/db/local-tasks.ts`
- כתיבה: cache מיידי (UI אינסטנט) **וגם** POST דורابيל ל-`/api/tasks`.
- קריאה: `syncTasksFromDb()` ב-mount מיישר את ה-cache מול ה-DB (האמת, per-user).
- ה-cache עכשיו **per-user** (`pmo_added_tasks::<uid>`) — סוגר דליפה אפשרית בין משתמשים בדפדפן משותף;
  ה-cache הישן (single-user) מהגר פעם אחת לתחום של u1.
- `local-task-detail.tsx` טוען DB-first ונופל ל-cache. `intake-workflow.tsx` שולח את כל המשימות בכתיבה אחת.
  `role-context.tsx` מיישר מול ה-DB ב-mount.

### Degradation
ללא `DATABASE_URL` הכל יורד יפה ל-503 → cache בלבד (מצב mock כמו קודם).

### אומת מקצה-לקצה מול Neon
create→200 (creatorId מהסשן), list/get→200, **בידוד per-user** (ניר: 0 משימות + 403, חגי: רואה דרך
תת-העץ + 200), delete→200 ואז 404. tsc + production build נקיים. דף /tasks מרנדר 25 משימות עם תאריכי dd/mm/yyyy.
