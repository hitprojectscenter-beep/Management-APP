...
export function extractTasksHeuristic(text: string): ExtractedTask[] {
  ...
  // STRICT MODE anchors (user decision): an action only counts as a task if it
  // also carries an anchor — who (named owner), when (due date), or what
  // (concrete deliverable). An impersonal normative recommendation
  // ("יש ל/צריך/כדאי/חשוב ל + verb") with NO anchor is a reflection, not a
  // task — this is what stops a review/reflection document from producing
  // phantom tasks ("יש לבחון את התהליך" → not a task).
  const IMPERSONAL_NORMATIVE = /(?:^|[\s,.;:"'(])(?:יש|צריך|נדרש|כדאי|מומלץ|רצוי|חשוב)\s+ל/;
  // First-person commitments carry an implicit owner (speaker → current user),
  // so they qualify on their own: "אבדוק", "נכין", "נעביר את הנתונים".
  const FIRST_PERSON_FUTURE =
    /(?:^|[\s,.;:"'(])(?:א|נ)(?:כין|כתוב|בדוק|עביר|בצע|פנה|טפל|ארגן|תאם|חזיר|עדכן|שלח|סכם|ביא|ענה|קבע|כנס|סקור|פרסם|הכין|ודא|גיש|ציג)/;
  // A named artifact someone must produce = a concrete deliverable anchor.
  const DELIVERABLE_RE =
    /(?:מצגת|דו"?ח|מסמך|אפיון|מכתב|סיכום|תכנית|תוכנית|מפרט|הצעה|נוהל|טיוטה|דרישות|חוזה|מכרז|presentation|report|document|deck|spec|draft|proposal|brief|contract)/i;
  ...
  for (const line of lines) {
    ...
    // STRICT gate: a normative recommendation needs an anchor (owner / due /
    // deliverable / first-person commitment) to be a real task; otherwise it
    // is reflection and is dropped.
    const hasAnchor =
      !!responsibleHint || !!dueDate || DELIVERABLE_RE.test(line) || FIRST_PERSON_FUTURE.test(line);
    if (IMPERSONAL_NORMATIVE.test(line) && !hasAnchor) continue;
...
function buildGeminiExtractionPrompt(text: string, isFirstChunk: boolean): string {
  ...
  return `אתה אנליסט פגישות בכיר. תפקידך: לקרוא טקסט חופשי — סיכום פגישה, מסמך, תעתיק שמע — ולחלץ ממנו אך ורק את **המשימות הברורות לביצוע**.

**הגדרת משימה (מחמירה):** פעולה קונקרטית שמישהו צריך לבצע, המנוסחת ב**עתיד** ("ניר יכין"), ב**ציווי** ("הכן את הדוח") או כ**התחייבות** ("אבדוק ואחזור").
**מבחן חובה — שני התנאים חייבים להתקיים יחד:**
(1) יש **פועל-פעולה** בעתיד/ציווי/התחייבות; וגם
(2) יש לפחות **עוגן** אחד: מבצע מפורש (מי — שם אדם), תאריך יעד מפורש (מתי), או תוצר בעל-שם (מסמך / מצגת / דוח / אפיון). **שים לב:** עצם פעולת חקירה כללית ("לבדוק את הסיבות", "לבחון את האפשרות", "לקדם את הנושא") **אינה** "תוצר" — חייב להופיע אחד מהשלושה במפורש, אחרת זו המלצה.
אם פריט הוא אמירה **נורמטיבית-סתמית** ("יש ל...", "צריך ל...", "כדאי", "חשוב ל...", "מומלץ", "רצוי") בלי מבצע, בלי יעד ובלי תוצר — זו **המלצה/רפלקציה, לא משימה**. אל תכלול אותה.
אם המסמך כולו הוא רקע / סקירה / דעה / רפלקציה ואין בו אף פעולה מחויבת — החזר \`"tasks": []\` ריק. **עדיף 0 משימות אמיתיות מאשר להמציא משימות מתוך תיאור.**
...
החזר JSON בלבד (ללא טקסט נוסף) בפורמט הזה בדיוק:
{
  ...
  "tasks": [
    {
      "title": "כותרת = ה**פעולה** עצמה (שם-פעולה), לא הנושא ולא מצב העניינים. עד 5-6 מילים. דוגמאות טובות: 'הכנת דוח רבעוני', 'תיאום פגישה עם הספק', 'בדיקת אינטגרציית Salesforce'. אסור נושא/מצב ('השבתת פרויקט X', 'ניהול הסיכונים') — תרגם לפעולה ('בדיקת סיבות השבתת פרויקט X'). אסור ציטוט פתיחה שיחתית ('אהלן, ערב טוב, התבקשתי...')",
      "description": "2-5 משפטים המסבירים את ההקשר המלא של המשימה מתוך המקור: מה צריך לעשות, למה (הרקע/הסיבה מהפגישה), עבור מי, ותלויות אם יש. **חובה שיהיה שונה ועשיר יותר מהכותרת** — אסור שהתיאור יהיה זהה לכותרת או משפט בודד שחוזר עליה. העתק את ההקשר מהמקור (המשפטים שמסביב למשימה) ושלב אותם. אם המקור קצר — הרחב מעט מההקשר הסובב",
...
**מה כן משימה (חייב: פעולה + לפחות עוגן אחד — מי/מתי/מה):**
✓ פעולה עם מבצע מפורש: "ניר יכין מצגת", "אסתר תתאם פגישה עם הספק"
✓ פעולה עם תאריך יעד: "להגיש את הדוח עד יום חמישי", "נעדכן את הלקוח עד סוף השבוע"
✓ פעולה עם תוצר קונקרטי: "להפיק מסמך אפיון", "נדרשת מצגת לדירקטוריון"
✓ התחייבות אישית: "אבדוק ואחזור", "ניקח על עצמנו להעביר את הנתונים"
✓ החלטה שמובילה לפעולה מיידית עם מבצע/יעד: "סוכם שאלעד יפנה לספק עד ראשון"

**מה לא משימה (אל תכלול בשום אופן):**
✗ **אמירה נורמטיבית-סתמית בלי מבצע/יעד/תוצר**: "יש לבחון את התהליך", "כדאי לשפר את התקשורת", "חשוב להגדיר ציפיות", "יש לקדם את הנושא" — המלצה/רפלקציה, לא משימה
✗ **פעולת חקירה/בחינה כללית בלי עוגן**: "יש לבדוק את הסיבות להשבתה", "יש לבחון את האפשרות להחזיר לפעילות", "יש ליישם תהליך סדור" — בלי שם-אחראי, בלי תאריך-יעד ובלי תוצר בעל-שם = רפלקציה, **לא** משימה. (אם נאמר "אלעד יבדוק את הסיבות עד ראשון" — זו כן משימה.)
✗ רקע / כללי / מבוא / הנדון — תיאור הקשר
✗ שורות סטאטוס / עדכון מצב ("סטאטוס פעילות IR")
✗ כותרות וכותרות-משנה, אותיות/מספרי סעיף ("א", "ii")
✗ מה שכבר קרה (עבר): "נפגשנו עם הלקוח", "התקיימה פגישה", "מארק סקר את הנושא"
✗ תיאור מצב הווה: "הפרויקט אינו פעיל", "הנושא מתמסמס"
✗ דעות, אבחנות, חזון/אסטרטגיה ברמת-על

**מבחן 'מי / מה / מתי':** אם אי אפשר למלא לפחות שניים מהשלושה — זו לא משימה.
- "ניהול הסיכונים מתמסמס" → לא (תיאור מצב, אין פעולה מחויבת)
- "יש לשפר את ניהול הסיכונים" → לא (נורמטיבי בלי מבצע/יעד/תוצר)
- "אלעד יציג את ממצאי המבדק עד חמישי" → כן (מי=אלעד · מה=הצגת ממצאים · מתי=חמישי) ✓
