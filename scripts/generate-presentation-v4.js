/**
 * Generate PMO++ presentation v4 — Hebrew typography polish.
 *
 * Updates vs v3:
 *  - heb() helper auto-converts:
 *      • ASCII " between Hebrew letters → Hebrew gershayim ״ (לו"ז → לו״ז)
 *      • ASCII ' after Hebrew letter   → Hebrew geresh ׳ (וכו' → וכו׳)
 *      • Latin runs are wrapped with LRM marks so words like "PMO++"
 *        keep their LTR order inside an RTL paragraph
 *      • Parenthesized Latin (English) → wrapped with LRI/PDI isolates
 *        so "(API)" displays as "(API)" instead of ")API(" in RTL.
 *  - All Hebrew text auto-gets rtlMode:true + lang:"he-IL" via the helpers.
 *  - English subtitles (subtitle line under each title) stay LTR.
 *
 * Run: node scripts/generate-presentation-v4.js
 */
...
// ============================================================
// Hebrew typography helper
// ============================================================
const LRM = "‎"; // forces LTR — for embedded English in Hebrew text
const RLM = "‏"; // forces RTL
const LRI = "⁦"; // start LTR isolate — for parenthesized Latin
const PDI = "⁩"; // end isolate
...
function heb(text) {
  ...
  // 1) Hebrew abbreviation gershayim: לו"ז → לו״ז, מנכ"ל → מנכ״ל
  t = t.replace(/([֐-׿])"([֐-׿])/g, "$1״$2");
  // 2) Hebrew geresh: וכו' → וכו׳, ר' → ר׳
  t = t.replace(/([֐-׿])'/g, "$1׳");

  // 3) Parenthesized Latin/digit runs get LTR isolate so "(API)" renders
  //    as "(API)" not ")API(" in RTL paragraph.
  //    Match (...) where content has at least one Latin letter or digit.
  t = t.replace(/\(([^()֐-׿]*[A-Za-z0-9][^()֐-׿]*)\)/g,
    LRI + "($1)" + PDI);

  // 4) Wrap Latin runs (English words, product names, version numbers)
  //    with LRM so they hold LTR order even with adjacent Hebrew or
  //    punctuation. Skip if already inside an isolate (handled in step 3).
  //    Matches: identifiers, version numbers, hyphenated terms, dots.
  t = t.replace(
    /([A-Za-z][A-Za-z0-9.+_/\-]*)/g,
    (m, _g, offset, full) => {
      // Skip if already wrapped (preceded by LRI or LRM)
...
// ============================================================
// Smart text wrappers — automatic Hebrew handling
// ============================================================
function addTxt(slide, text, opts) {
  // If text is an array (for bullets), process each item
  ...
  if (isHe) {
    finalOpts.rtlMode = true;
    finalOpts.lang = "he-IL";
  }

  let finalText;
  if (Array.isArray(text)) {
    finalText = text.map((item) => {
      if (typeof item === "string") return heb(item);
      return { ...item, text: heb(item.text) };
    });
  } else {
...
// ============================================================
{
  ...
  addTxt(s, "המרכז למיפוי ישראל · Israel Mapping Center", { x: 0.6, y: 5.2, w: 12.1, h: 0.5, fontSize: 22, color: C.accent, fontFace: F.header, bold: true, align: "center", margin: 0 });
  addTxt(s, "מצגת מקיפה · 20 פרקים · גרסה 4 · 2026", { x: 0.6, y: 6.4, w: 12.1, h: 0.4, fontSize: 14, color: C.primaryLight, fontFace: F.body, align: "center", margin: 0 });
}
...
// ============================================================
{
  ...
  statBox(s, 0.6, 1.8, 2.7, 1.3, "17", "מדדי KPI", C.primary);
  statBox(s, 3.5, 1.8, 2.7, 1.3, "PM · PMO", "בורר תפקיד", C.indigo);
  statBox(s, 6.4, 1.8, 2.7, 1.3, "EVM", "CPI · SPI", C.purple);
  statBox(s, 9.3, 1.8, 3.4, 1.3, "RAG", "בקרה למנהלים", C.success);
  ...
  section(s, 0.6, 5.2, 12.1, 1.8, "מקורות + אינטראקטיביות", "כל כרטיס לחיץ — בועת מידע מפורטת עם הסבר, נוסחה, מקור וגרף לדוגמה. מבוסס תקני PMBOK, EVM, McKinsey, Bain, MBI ו-Gartner. שני דשבורדים מובחנים ויזואלית (כחול PM / סגול PMO).", C.accent);
  footer(s);
}
...
// ============================================================
{
  ...
  section(s, 0.6, 1.8, 12.1, 1.4, "עץ היררכי רקורסיבי", "אגרגציה מ-leaf nodes למעלה דרך כל ההיררכיה: שעות מתוכננות/בפועל, אחוז התקדמות משוקלל לפי שעות, תאריכים מינימלי/מקסימלי, עלויות (תעריף שעתי × שעות), וספירת משימות (total/done/blocked/overdue).", C.primary);
  ...
  section(s, 0.6, 5.1, 12.1, 1.9, "Roll-up bars בגאנט", "פסי אב מציגים summary של ה-node כולו: תאריך מינימלי של leaf, תאריך מקסימלי של leaf וממוצע התקדמות משוקלל. לחיצה על node פותחת פופאפ עם משימות / הושלמו / חסומות / באיחור / שעות / תוצר.", C.success);
  footer(s);
}
...
// ============================================================
// SLIDE 8 — Google Calendar Integration
// ============================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  ...
  // Hero: two-way sync
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 0.6, y: 1.8, w: 5.4, h: 1.4, fill: { color: C.primary }, line: { color: C.primary, width: 0 }, rectRadius: 0.15 });
  s.addText("PMO++", { x: 0.6, y: 1.85, w: 5.4, h: 0.55, fontSize: 24, color: C.white, fontFace: F.header, bold: true, align: "center", margin: 0 });
  addTxt(s, "משימות · אבני דרך · פגישות", { x: 0.6, y: 2.5, w: 5.4, h: 0.45, fontSize: 13, color: C.primaryLight, fontFace: F.body, align: "center", margin: 0 });
  ...
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 7.3, y: 1.8, w: 5.4, h: 1.4, fill: { color: C.success }, line: { color: C.success, width: 0 }, rectRadius: 0.15 });
  s.addText("Google Calendar", { x: 7.3, y: 1.85, w: 5.4, h: 0.55, fontSize: 24, color: C.white, fontFace: F.header, bold: true, align: "center", margin: 0 });
  s.addText("Events · Reminders · Attendees", { x: 7.3, y: 2.5, w: 5.4, h: 0.45, fontSize: 13, color: C.white, fontFace: F.body, align: "center", margin: 0 });

  section(s, 0.6, 3.4, 6, 1.7, "Push: PMO++ ⟵ Google", "כל משימה חדשה או עדכון סטטוס יוצרים אוטומטית אירוע ביומן של האחראי. שינוי תאריך — עדכון האירוע.", C.primary);
  ...
  section(s, 0.6, 5.2, 6, 1.8, "OAuth 2.0", "התחברות Google רגילה. ה-scope הוא calendar.events. ה-token נשמר ב-DB מוצפן. רענון אוטומטי.", C.indigo);
...
// ============================================================
{
  ...
  section(s, 0.6, 3.7, 6, 1.5, "חיזוי", "Velocity actual מול required + ממוצע סליפ — forecast end date.", C.primary);
  section(s, 6.8, 3.7, 5.9, 1.5, "אפקט שרשרת", "BFS על גרף תלויות — מספר משימות מושפעות, מתוכן בנתיב קריטי, וימי cascade.", C.purple);
...
// ============================================================
{
  ...
  section(s, 0.6, 1.8, 6, 5.1, "AI Sidekick (Chat)",
    ...
    C.primary);
  section(s, 6.8, 1.8, 5.9, 5.1, "תכנית גידור (Mitigation Plan)",
...
// ============================================================
{
  ...
  section(s, 0.6, 5.5, 12.1, 1.5, "אופן הבנייה", "3 שלבים ויזואליים: Trigger ← Conditions ← Actions. כרטיס סיכום סגול עם ולידציה ו-toast. אינטגרציה עם 2 פעולות AI (summarize, recommend).", C.purple);
  footer(s);
}

// ============================================================
// SLIDE 14 — Team & Email Invitations
// ============================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  ...
  // Email mockup
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 6.4, y: 1.7, w: 6.3, h: 3.5, fill: { color: C.offWhite }, line: { color: C.danger, width: 2 }, rectRadius: 0.15 });
  s.addShape(pres.shapes.RECTANGLE, { x: 6.4, y: 1.7, w: 6.3, h: 0.55, fill: { color: C.danger }, line: { color: C.danger, width: 0 } });
  addTxt(s, "📧 מייל הזמנה (Demo)", { x: 6.4, y: 1.75, w: 6.3, h: 0.45, fontSize: 13, color: C.white, fontFace: F.header, bold: true, align: "center", margin: 0 });
  s.addText("From: noreply@mapi.gov.il\nTo: new.user@example.com\nSubject: הזמנה ל-PMO++ של המרכז למיפוי", { x: 6.6, y: 2.35, w: 6.0, h: 0.85, fontSize: 10, color: C.muted, fontFace: "Courier New", align: "right", margin: 0, rtlMode: true, lang: "he-IL" });
  addTxt(s, "שלום,\nהוזמנת להצטרף ל-PMO++ של המרכז למיפוי ישראל.\n\nתפקיד: מנהל פרויקט\nחטיבה: IT · אגף: יישומים", { x: 6.6, y: 3.2, w: 6.0, h: 1.05, fontSize: 11, color: C.dark, fontFace: F.body, align: "right", margin: 0 });
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 8.4, y: 4.35, w: 2.3, h: 0.5, fill: { color: C.primary }, line: { color: C.primary, width: 0 }, rectRadius: 0.08 });
  addTxt(s, "⚡ אישור הזמנה", { x: 8.4, y: 4.35, w: 2.3, h: 0.5, fontSize: 11, color: C.white, fontFace: F.header, bold: true, align: "center", valign: "middle", margin: 0 });

  section(s, 0.6, 3.1, 5.6, 2.1, "טופס הזמנת חבר",
...
// ============================================================
{
  ...
  for (const t of tabs) {
    ...
    addTxt(s, t.name, { x: x + 0.3, y: y + 0.2, w: 3.5, h: 0.5, fontSize: 16, color: t.color, fontFace: F.header, bold: true, align: "right", margin: 0 });
    s.addText(t.en, { x: x + 0.3, y: y + 0.65, w: 3.5, h: 0.35, fontSize: 11, color: C.muted, fontFace: F.body, italic: true, align: "right", margin: 0 });
    addTxt(s, t.desc, { x: x + 0.3, y: y + 1.05, w: 3.5, h: 1.2, fontSize: 11, color: C.dark, fontFace: F.body, align: "right", margin: 0, valign: "top" });
    i++;
  }
  footer(s);
}
...
// ============================================================
// SLIDE 16 — RBAC: Role-Based Visibility Matrix
// ============================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  ...
  addTxt(s, "הרשאה", { x: tableX + 0.1, y: tableY, w: labelW - 0.2, h: headerH, fontSize: 11, color: C.white, fontFace: F.header, bold: true, align: "right", valign: "middle", margin: 0 });
  ...
  for (let r = 0; r < permissions.length; r++) {
    ...
    addTxt(s, permissions[r].he, { x: tableX + 0.15, y, w: labelW - 0.25, h: rowH, fontSize: 10, color: C.dark, fontFace: F.body, align: "right", valign: "middle", margin: 0 });
    for (let c = 0; c < 5; c++) {
...
// ============================================================
// SLIDE 17 — Notifications & Email System
// ============================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  ...
  section(s, 0.6, 3.8, 6, 3.2, "🚀 מיילים שנשלחים אוטומטית",
    ...
    C.primary);
  section(s, 6.8, 3.8, 5.9, 3.2, "⚙️ ניהול העדפות התראה",
...
// ============================================================
{
  ...
  section(s, 6.8, 4.2, 5.9, 1.4, "RTL מלא", "Logical properties: ms / me / ps / pe / start / end. הגאנט בנוי ידנית כדי לתמוך ב-RTL.", C.purple);
...
// ============================================================
{
  ...
  section(s, 0.6, 1.9, 6, 2.3, "תפריט נשלף (Sheet)", "Radix Sheet primitive. תפריט המבורגר ☰ במובייל. ניווט slide-in RTL. ה-Topbar מצמיד hamburger button.", C.primary);
  section(s, 6.8, 1.9, 5.9, 2.3, "Touch targets", "כל כפתור min-h-[44px] min-w-[44px] לפי Apple HIG. inputs בעלי font-size: 16px למניעת zoom ב-iOS. onFocus scroll into view.", C.indigo);
  section(s, 0.6, 4.3, 6, 2.5, "PWA Manifest", "שם עברי. theme color כחול Mapi. standalone mode. icons 192/512 PNG. apple-touch-icon.\n\nהתקנה: Chrome → Install app, Safari → Add to Home Screen.", C.purple);
...
// ============================================================
{
  ...
  addTxt(s, "Backend ואינטגרציות", { x: 4.8, y: 1.9, w: 4.0, h: 0.5, fontSize: 18, color: C.white, fontFace: F.header, bold: true, align: "center", margin: 0 });
  ...
  addTxt(s, "ייצור: Neon Postgres / Supabase / Vercel Postgres — חיבור דרך drizzle() client. פיתוח: Mock data layer (lib/db/mock-data.ts) עם נתוני Salesforce אמיתיים מהאקסל.", { x: 1.0, y: 6.25, w: 11.3, h: 0.55, fontSize: 11, color: C.white, fontFace: F.body, italic: true, align: "right", margin: 0 });

  s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 7.0, w: 13.3, h: 0.5, fill: { color: C.dark }, line: { color: C.dark, width: 0 } });
  addTxt(s, "Hosting: Vercel · Source: GitHub · CI/CD: auto deploy on push to main · OAuth 2.0 ל-Google Calendar", { x: 0.6, y: 7.1, w: 12.1, h: 0.35, fontSize: 11, color: C.white, fontFace: F.body, align: "center", margin: 0 });
}

// ============================================================
// Save
// ============================================================
const outPath = path.join(__dirname, "..", "PMO-Plus-Plus-Comprehensive-Presentation-v4.pptx");
